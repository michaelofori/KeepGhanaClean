    ...
    private void startPluginForegroundService(Bundle extras) {
        ...
        switch (importance) {
            ...
            default:
                ...
                // We are not using IMPORTANCE_MIN because we want the notification to be
                // visible
        }

        // Create notification channel
        NotificationChannel channel = new NotificationChannel("foreground.service.channel", "Background Services",
                importance);
        channel.setDescription("Enables background processing.");
        getSystemService(NotificationManager.class).createNotificationChannel(channel);

        // Get notification icon
        // int icon = getResources().getIdentifier((String) extras.get("icon"),
